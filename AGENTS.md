...
This file contains instructions and context for AI agents (and humans) working on the `@melledijkstra/monorepo` repository.
...
- **Package Manager:** Uses `pnpm`. Ensure you have the version specified in `package.json`.
- **Node.js:** Ensure a compatible Node.js version is installed (see `.nvmrc`).
